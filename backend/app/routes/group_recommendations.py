...
group_recommendations_bp = Blueprint('group_recommendations', __name__)

@group_recommendations_bp.route('/recommended-groups', methods=['GET'])
def get_recommended_groups():
    ...
    # Get recommended groups based on activity and member count
    recommended_groups = db.session.query(
        Group.group_id,
        Group.group_name,
        Group.description,
        Group.created_at,
